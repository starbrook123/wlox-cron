...
if ($result[0]['last_date'] == date('Y-m-d')) {
	...
	$result = db_query_array($sql);
	if ($result) {
		db_insert('shares_summary',array('date'=>date('Y-m-d'),'shares_held'=>$shares_info['shares_held'],'buy_total_usd'=>$result[0]['buy_usd'],'sell_total_usd'=>$result[0]['sell_usd'],'dividends_usd'=>$result[0]['dividends_usd'],'status_id'=>1));
